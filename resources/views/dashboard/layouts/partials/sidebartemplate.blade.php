        ...
        <a href="/profile" class="flex items-center  px-4 -mx-2">
            ...
            <div class=" ">
                <div class="flex items-center gap-2">
                    <span class="mx-2 font-medium text-gray-800 dark:text-gray-200 line-clamp-1">{{
                        Auth::user()->username }}</span>
                    @include('layouts.partials.verified')
                </div>
                <p
                    class="px-3 py-1 text-xs {{ auth()->user()->role->role === 'Admin' ? 'text-red-500 bg-red-100/60' : 'text-indigo-500 bg-indigo-100/60' }}  rounded-full dark:bg-gray-800 ">
                    {{ auth()->user()->role->role }}
                </p>
            </div>
        </a>
